...
export default function Unlock() {
    ...
    const handleUnlock = async () => {
        setLoading(true);
        try {
            // Using logic associated with backend's hardcoded credentials for demo
            await api.login('testuser', 'dummy-proof');
            setLocation('/dashboard');
        } catch (error) {
            console.error(error);
            alert('Unlock failed: Invalid credentials or server error');
        } finally {
            setLoading(false);
        }
    };
